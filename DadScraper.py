...
def extract_data(soup,Desc):
	...
	for tr in tbl.findAll('tr')[3:]: #Tinker Records
		...
		info['City']=City
		info['Phone']=Phone	
		url ='http://www.virginiaequestrian.com/{}'.format(url.a['href'])
...
def main():
	url='http://www.virginiaequestrian.com/main.cfm?action=greenpages'
	r=requests.get(url)
	soup=BeautifulSoup(r.content)
	List = soup.findAll('table')[2]
	data=[]
	for tr in List.find_all('tr')[0:]: #Tinker Categories
		url='http://www.virginiaequestrian.com/{}'.format(tr.a['href'])
		r=requests.get(url)
		soup=BeautifulSoup(r.content)
		Desc=tr.a.string.strip()		
		step= extract_data(soup,Desc)
		data=data+step
		keys = data[0].keys()
	if os.path.exists('VirginiaEQData.csv'):
		os.remove('VirginiaEQData.csv')	
		with open('VirginiaEQData.csv','wb') as f:
